...
bool QAccordion::swapContentPane(uint index, ContentPane* cpane)
{
    if (this->checkIndexError(index, false,
            "Can not swap content pane at index " + QString::number(index) + ". Index out of range.")) {
        return false;
    }
...
bool QAccordion::moveContentPane(uint currentIndex, uint newIndex)
{
    if (this->checkIndexError(currentIndex, false,
            "Can not move from " + QString::number(currentIndex) + ". Index out of range.") || this->checkIndexError(newIndex, false, "Can not move to " + QString::number(newIndex) + ". Index out of range.")) {
        return false;
    }
    ...
    ContentPane* cpane)
{
    if (this->checkIndexError(
            index, true, "Can not insert Content Pane at index " + QString::number(index) + ". Index out of range")) {
        return false;
    }
...
bool QAccordion::internalRemoveContentPane(bool deleteOject, int index,
    ...
    ContentPane* cpane)
{
    if (index != -1 && this->checkIndexError(index, false, "Can not remove content pane at index " + QString::number(index) + ". Index out of range")) {
        return false;
    }
    ...
    ContentPane* cpane)
{
    ...
    if (cframe != nullptr) {
        auto result = std::find_if(this->contentPanes.begin(), this->contentPanes.end(),
